...
export type AccountReaderFunction = (
    accountIdx: number
...
export class DiscoveryConfig implements IDiscoveryConfig {
    ...
    constructor(config: IDiscoveryConfig) {
        ...
        this.mnemonicLanguage =
            config.mnemonicLanguage ?? TMnemonicLanguage.ENGLISH;
...
export class SmartWalletDiscovery {
     ...
     * @param password
     */
    public async discoverAccountsFromMnemonic(
        config: DiscoveryConfig,
        mnemonic: string,
        password?: string
    ): Promise<void> {
        const hdRootKey: EthereumHDKey =
            SmartWalletDiscovery.getRootExtKeyFromMnemonic(
                mnemonic,
                password,
                config.mnemonicLanguage
            );

        let ended = false;
        let currentAccount = 0;
        const smartWalletFactory = new this.web3.eth.Contract(
            ISmartWalletFactory.abi,
            config.factory
        );
        const creationByteCode = await smartWalletFactory.methods
            .getCreationBytecode()
            .call();
        ...
        while (!ended) {
            ...
            while (currentGap <= gapLimit) {
                const walletKey = hdRootKey.derivePath(
                    `${address}/${currentAccount}'/0/${index}`
                );
                const currentAddress = walletKey.getWallet().getAddressString();

                const activityFound = await this.searchTrx(
                    currentAddress,
                    smartWalletFactory
                );
                const discoveredAccount: DiscoveredAccount = {
                    eoaAccount: constants.ZERO_ADDRESS,
                    swAccounts: []
                };
                ...
                while (currentSWGap <= config.sWalletGap) {
                    ...
                    const currentSWAddress = config.isCustomWallet
                        ? this.calculateCustomSmartWalletAddress(
                              currentAddress,
                              config.recoverer,
                              logicAddress,
                              initParamsHash,
                        ...
                        : this.calculateSmartWalletAddress(
                              currentAddress,
                              config.recoverer,
                              walletIndex,
                              config.factory,
                    ...
                    const swAtivityFound = await this.searchTrx(
                        currentSWAddress,
                        smartWalletFactory
                    );
                ...
                if (
                    discoveredAccount.swAccounts.length > 0 &&
                    discoveredAccount.eoaAccount === constants.ZERO_ADDRESS
                ) {
                    discoveredAccount.eoaAccount = currentAddress;
                }
     ...
     */
    public async discoverAccountsFromExtendedPublicKeys(
        config: DiscoveryConfig,
        extendedPublicKeys: string[]
    ): Promise<void> {
        const smartWalletFactory = new this.web3.eth.Contract(
            ISmartWalletFactory.abi as any,
            config.factory
        );
        const creationByteCode = await smartWalletFactory.methods
            .getCreationBytecode()
        ...
        for (let i = 0; i < extendedPublicKeys.length; i++) {
            ...
            const hdAcckey: EthereumHDKey =
                EthereumHDKey.fromExtendedKey(extPubKey); // Account key (last hardened key)
            ...
            while (currentGap <= gapLimit) {
                ...
                const currentPubKey: EthereumHDKey =
                    hdAcckey.deriveChild(index);
                const currentAddress = currentPubKey
                    .getWallet()
                    .getAddressString();
                ...
                while (currentSWGap <= config.sWalletGap) {
                    ...
                    const swActivityFound = await this.searchTrx(
                        currentSWAddress,
                        smartWalletFactory
                    );
    ...
    async discoverAccounts(
        config: DiscoveryConfig,
        accountReader?: AccountReaderFunction
    ): Promise<void> {
        let ended = false;
        let currentAccount = 0;
        const smartWalletFactory = new this.web3.eth.Contract(
        ...
        while (!ended) {
            ...
            const accountReaderFunc =
                accountReader ?? this.getAccountExtendedPublicKey;

            const currentAccountPubK = await accountReaderFunc(currentAccount);

            if (
                currentAccountPubK === undefined ||
                currentAccountPubK === null
            ) {
                ended = true;
                break;
            }
            ...
            const hdAcckey: EthereumHDKey =
                EthereumHDKey.fromExtendedKey(currentAccountPubK); // Account key (last hardened key)
    ...
    public static getRootExtKeyFromMnemonic(
        mnemonic: string,
        password?: string,
        language?: TMnemonicLanguage
    ): EthereumHDKey {
        let wordlist: string[];
    ...
    async getAccountExtendedPublicKey(
        ...
        console.debug('Account index', accountIdx);
    ...
    private async searchTrx(
        ...
        try {
            ...
            activityFound = new BN(
                await this.web3.eth.getBalance(currentAddress)
            ).gtn(0);
        } catch (error) {
        ...
        if (!activityFound) {
            try {
                ...
                activityFound =
                    (await this.web3.eth.getTransactionCount(currentAddress)) >
                    0;
            } catch (error) {
                await this.reconnect();
                activityFound =
            ...
            if (!activityFound) {
                ...
                while (
                    ...
                    errorCount < this.reconnectionAttempts
                ) {
                    try {
                        const tokenBalance = await this.web3.eth.call({
                            to: this.tokens[i],
                            data: balanceFunction
                        });
    ...
    public calculateSmartWalletAddress(
        ownerEOA: string,
        recoverer: string,
        walletIndex: number,
        factory: string,
    ...
    public calculateCustomSmartWalletAddress(
        ownerEOA: string,
        recoverer: string,
        logicAddress: string,
        initParamsHash: string,
        walletIndex: number,
        factory: string,
        bytecodeHash: string,
        chainId: number
