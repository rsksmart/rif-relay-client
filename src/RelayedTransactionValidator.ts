...
export default class RelayedTransactionValidator {
    ...
    validateRelayResponse(
        ...
        log.info(
            'validateRelayResponse - Transaction is',
            {
                v,
                r,
                s,
        ...
        const isDeploy = 
            (request as DeployTransactionRequest).relayRequest.request
                .recoverer ? true : false;
        ...
        if(relayRequestAbiEncode !== data){
            throw new Error('Relay request Encoded data must be the same as Transaction data');
        }

        if(relayWorker.toLowerCase() !== from.toLowerCase()){
            throw new Error('Informed relayWorker address must be the same as configured relayHubAddress');
        }
