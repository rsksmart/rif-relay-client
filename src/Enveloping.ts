...
export default class Enveloping {
     ...
     */
    async estimateDestinationContractInternalCallGas(
        forwarder: Address,
        to: Address,
    ...
    async createRelayRequest(
        ...
        if (gasToSend === undefined || gasToSend == null) {
            const internalCallCost =
                await this.estimateDestinationContractInternalCallGas(
                    forwarder,
                    to,
                    data,
                    gasPriceToSend
     ...
     */
    async generateDeployTransactionRequest(
        signature: PrefixedHexString,
     ...
     */
    async generateRelayTransactionRequest(
        signature: PrefixedHexString,
    ...
    async generateMetadata(
        ...
        const metadata: RelayMetadata = {
            ...
            signature: signature,
            relayMaxNonce:
                (await this.dependencies.contractInteractor.getTransactionCount(
                    this.relayWorkerAddress
                )) + this.config.maxRelayNonceGap
        };
    ...
    async getSenderNonce(sWallet: Address): Promise<IntString> {
        return await this.dependencies.contractInteractor.getSenderNonce(
            sWallet
        );
    }

    async getFactoryNonce(
        factoryAddr: Address,
        from: Address
    ): Promise<IntString> {
        return await this.dependencies.contractInteractor.getFactoryNonce(
            factoryAddr,
            from
        );
    }
    ...
    async sendTransaction(
        relayUrl: string,
        request: DeployTransactionRequest | RelayTransactionRequest
    ): Promise<RelayingAttempt> {
        const httpClient = new HttpClient(new HttpWrapper(), {});
        try {
            const hexTransaction = await httpClient.relayTransaction(
                relayUrl,
                request
            );
            ...
            const transaction = new Transaction(
                hexTransaction,
                this.dependencies.contractInteractor.getRawTxOptions()
            );
     ...
     */
    public static async discoverAccountsUsingMnemonic(
        ...
        const swd = new SmartWalletDiscovery(
     ...
     * @returns - List of discovered accounts, discoveredAccount.eoaAccount contains the discovered EOA and discoveredAccount.swAccounts all the discovered Smart Wallets for that EOA
     */
    public static async discoverAccountsFromExtendedPublicKeys(
        config: DiscoveryConfig,
        provider: provider,
