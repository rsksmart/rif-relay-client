...
const randomBigNumberJs = (max: number) => BigNumberJs(Math.random() * max);
...
describe('RelayPricer', function () {
  ...
  describe('findAvailableApi', function () {
    ...
    it('should return available api(lowercase)', function () {
      const availableApi = pricer.findAvailableApi(RIF_SYMBOL.toLowerCase());
      assert.containsAllKeys(availableApi, ['api', 'tokens']);
    });
  ...
  describe('getExchangeRate', function () {
    ...
    beforeEach(function () {
      ...
      sourceApiStub.getApiTokenName.withArgs(RIF_SYMBOL).returns(RIF_SYMBOL);
      sourceApiStub.getApiTokenName.withArgs(RBTC_SYMBOL).returns(RBTC_SYMBOL);
    });
    ...
    it('Should return exchange rate using different exchanges', async function () {
      ...
      rdocApiStub.getApiTokenName.withArgs(RDOC_SYMBOL).returns(RDOC_SYMBOL);
      ...
      const price = await pricer.getExchangeRate(RDOC_SYMBOL, RBTC_SYMBOL);
